...
chip_size = 256  # Size of the NAIP image chip in pixels (e.g., 256x256)
uniform_dataset_name = f"Ailanthus_Uniform_BlockCV_PA_1300m_thin_NAIP_{chip_size}_Aug2925"
crossval_dataset_name = f"Ailanthus_CrossVal_BlockCV_PA_1300m_thin_NAIP_{chip_size}_Aug2925"
...
# Paths for Host Occurrence Data - ensure thinning resolution matches climate dataset
host_occurrence_fp = r"D:\Ailanthus_NAIP_Classification\Ailanthus_Occurrences\ailanthus_nc_inat_gbif_2015_2025_dedup_1300m.csv"
...
blocks_shapefile_path = r"D:\Ailanthus_NAIP_Classification\Ailanthus_Occurrences\Ailanthus_BlockCV_PA_1300m_Spatial_Blocks.shp"
...
n_absences = len(presence_unique) # Sample equal number of absences as presences (575)
...
# Split the presence/ pseudoabsence data into train, validation, and test sets by random division (uniform sampling)
...
geojson_out_fp = os.path.join(uniform_dataset_root, "Ailanthus_Pres_Pseudoabs_NC_Uniform_BlockCV_Train_Val_Test_Aug2925.geojson")
...
# Normalize the name you use later
...
unique_folds = sorted(pa_data["fold"].unique())  # expect [1,2,3,4,5]
cv_configs = []
...
# Save to GeoJSON
geojson_out_fp = os.path.join(crossval_dataset_root, "Ailanthus_Pres_Pseudoabs_NC_SpatialCV_Blocks_Aug2925.geojson")
...
for round_num in unique_folds: # e.g. 1,2,3,4,5
    ...
    out_fp = os.path.join(
        ...
        f"Ailanthus_Pres_Pseudoabs_NC_SpatialCV_Blocks_Fold_{round_num}_Aug2925.geojson"
    )
...
for cv_round in [1, 2, 3, 4, 5]:
    ...
    gdf = gpd.read_file(os.path.join(crossval_dataset_root, f"Ailanthus_Pres_Pseudoabs_NC_SpatialCV_Blocks_Fold_{cv_round}_Aug2925.geojson"))
    ...
    df_master = pd.DataFrame(master_records)
    csv_out_fp = os.path.join(cv_dir, f"Ailanthus_Train_Val_Test_BlockCV_{cv_round}.csv")
