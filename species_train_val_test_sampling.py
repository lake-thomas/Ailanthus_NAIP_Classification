...
def extract_worldclim_vars_for_point(lon, lat, worldclim_folder):
    """
...
# # Add 19 WorldClim columns to the presence/ pseudo-absence data
# for n in range(1, 20):
#     pa_data_split[f'wc_bio_{n}'] = np.nan

# # Iterate over each point in the dataset and extract WorldClim variables
# for idx, row in pa_data_split.iterrows():
#     lon, lat = row['lon'], row['lat']
#     wc = extract_worldclim_vars_for_point(lon, lat, worldclim_folder)
#     for k, v in wc.items():
#         pa_data_split.at[idx, k] = v

# # Preview the updated DataFrame with WorldClim variables
# print("WorldClim variables extracted:")
# print(pa_data_split.head())
...
for idx, row in pa_data_split.iterrows():
    ...
    try:
        ...
        if result:
            print(f"✅ Chip written: {out_fp}")
        else:
            print(f"⚠️ Missing coverage for {lon}, {lat}")
    except Exception as e:
        print(f"❌ Failed for point {lon}, {lat}: {e}")
...
for idx, row in tqdm.tqdm(pa_data_split.iterrows(), total=len(pa_data_split)):
    ...
    wc_vars = extract_worldclim_vars_for_point(lon, lat, worldclim_folder)
